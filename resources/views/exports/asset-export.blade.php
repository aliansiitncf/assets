...
<body>
    ...
    <!-- SUMMARY -->
    <div class="summary">
        <h4>Total Asset Recorded</h4>
        <span>{{ count($assets) }} Units</span>
    </div>
                        ...
                        <tr>
                            ...
                            <td width="110" valign="top">
                                <div class="asset-image">
                                    ...
                                    <img src="{{ public_path('storage/' . $asset->image_path) }}">
                                    @else
                            ...
                            <td valign="top" style="padding-left:12px">
                                ...
                                @if($asset->category)
                                <div class="badge">{{ strtoupper($asset->category->name) }}</div>
                                @endif

                                <div class="asset-info">
                                    <div><span>Purchase</span>:
                                        {{ optional($asset->purchase_date)->format('d F Y') ?? '-' }}</div>
                                    <div><span>Location</span>:
                                        {{ optional($asset->latestLocation)->location->name ?? '-' }}</div>
                                    <div><span>Status</span>: {{ $asset->condition }}</div>
                                </div>
                            </td>
                        </tr>
                    </table>
